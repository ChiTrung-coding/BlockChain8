...
const FundRiserCard = ({props,pushWithdrawRequests}) => {
  ...
  const contributeAmount = (projectId,minContribution) =>{
    ...
    if(amount < minContribution){
      toastError(`So tien dong gop toi thieu la ${minContribution}`);
      return;
    }
    ...
    const onSuccess = () =>{
      setBtnLoader(false)
      setAmount(0)
      toastSuccess(`Da dong gop thanh cong ${amount} ETH`)
    }
  ...
  const requestForWithdraw = (projectId) =>{
    ...
    const data = {
      description:`${amount} ETH yeu cau rut`,
      amount:contributionAmount,
      recipient:account,
      account:account
    }
    const onSuccess = (data) =>{
      ...
      toastSuccess(`Yeu cau rut tien thanh cong ${amount} ETH`)
    }
    const onError = (message) =>{
      setBtnLoader(false)
      toastError(message)
      ...
      <div className="inner-card my-6 w-full lg:w-3/5">
        {
          ...
          <>
            <label className="text-sm text-gray-700 font-semibold">So tien dong gop :</label>
            ...
            <p className="text-sm text-red-600"> <span className="font-bold">NOTE : </span> Dong gop toi thieu la {props.minContribution} ETH </p>
          </>
          :
          <>
            <p className="text-md font-bold font-sans text-gray">So du hop dong</p>
            <p className="text-sm font-bold font-sans text-gray-600 ">{props.contractBalance} ETH </p>

            {
              props.creator === account?
              <>
              <label className="text-sm text-gray-700 font-semibold">Yeu cau rut :</label>
              <div className="flex flex-row">
                <input type="number" placeholder="Type here" value={amount} onChange={(e)=>setAmount(e.target.value)} disabled={btnLoader === props.address} className="input rounded-l-md" />
